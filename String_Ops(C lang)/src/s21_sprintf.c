...
int processChar(char **str, va_list va, struct FormatData *formatData) {
  ...
  char *pointer = *str + s21_strlen(*str);

  formatData->width--;  // Уменьшаем ширину на 1, так как добавляем один символ

  // Выравнивание по левому краю
  if (formatData->flags.minus) {
    *pointer++ = c;
    *pointer = '\0';
    *str = pointer;
  ...
  formatData->counter++;  // Увеличиваем счетчик обработанных символов
  return 0;
}
...
int processString(char *str, va_list va, struct FormatData *formatData) {
  ...
  // Если не указана точность, то она равна длине строки
  if (!formatData->has_precision) {
    formatData->precision = len;
  }

  // Вычисляем количество пробелов/нулей для выравнивания
  if (formatData->width > formatData->precision) {
...
int processFloat(char *str, struct FormatData *formatData, va_list va) {
  long double value =
      formatData->length == 'L' ? va_arg(va, long double) : va_arg(va, double);
...
void writeE_Value(char *str, struct FormatData *formatData, long double value) {
  char valueStr[1024] = {'\0'};
  int exp_counter = 0;
  int inf_nan = checkIfNanOrInf(valueStr, formatData, value);
  if (fabsl(value) >= 1.0 && inf_nan == 0) {
    while (floorl(fabsl(roundl(value * powl(10, formatData->precision + 1)) /
                        powl(10, formatData->precision + 1))) >= 10) {
      value = value / 10;
      exp_counter++;
    }
  } else if (value != 0 && inf_nan == 0) {
    while (fabsl(value) < 1) {
      value = value * 10;
      exp_counter--;
    }
  ...
  flagMinusHandler(str, valueStr, formatData, value);
}
...
int processInt(char *str, struct FormatData *formatData, va_list va) {
  ...
  switch (formatData->length) {
    case 'h':
      res = (short int)va_arg(va, int);
      break;

    case 'l':
      res = (long long)va_arg(va, long long int);
      break;
...
int writeInt(char *str, struct FormatData *formatData, long long value) {
  ...
  flagMinusHandler(str, valueStr, formatData, (long double)value);
  return 0;
}
...
void intToStr(char *valueStr, long long int value,
  ...
  if (value == 0 &&
    ...
    while (value != 0) {
      char ost = (value % 10) * coef + 48;  // код ASCII
      valueStr[i] = ost;
      value /= 10;
      i--;
      formatData->counter++;
...
/*Левое выравнивание (с флагом minus):
Сначала устанавливаются знак и нули,
затем добавляется строка с числом, и
в конце устанавливаются пробелы

Правое выравнивание (без флага minus):
Сначала уменьшается ширина для знака и нулей,
затем устанавливаются пробелы,
 потом устанавливаются знак и нули, и
 в конце добавляется строка с числом*/
...
/*флаги + в начале
- выравнивание по левому краю |9999   |
' ' для пол чисел вместо + пробел
# '0x' '0'
0 |000009|
*/
...
int setSign(char *str, long double value, struct FormatData *formatData) {
  if (formatData->specifier != 'u') {
    if (value < 0) {
      ...
      s21_strcat(str, "+");
      formatData->counter++;
    } else if (formatData->flags.space != 0) {
      s21_strcat(str, " ");
      formatData->counter++;
    }
  }
...
void writeG_Value(char *str, struct FormatData *formatData, long double value) {
  ...
  long double absValue = fabsl(value);
  long double scaledValue = absValue * powl(10, 4);

  if ((absValue / powl(10, formatData->precision) > 1) ||
      (scaledValue < 1 && value != 0)) {
    if (formatData->precision > 0) {
      formatData->precision--;
    }
    writeE_Value(str, formatData, value);
  } else {
    if (absValue < 0.1 && value != 0) {
      int j = 1;
      if (formatData->precision == 0) {
        formatData->precision++;
      }
      while (absValue * powl(10, j) < 1) {
        formatData->precision++;
        j++;
      }
    } else if (value != 0) {
...
int processUnsigned(char *str, struct FormatData *formatData, va_list va) {
  ...
  switch (formatData->length) {
    case 'h':
      value = (unsigned short)va_arg(va, unsigned int);
      break;
    case '\0':
...
int unsignedToStr(char *valueStr, unsigned long long value,
                  struct FormatData *formatData) {
  if (value == 0 &&
      (formatData->has_precision == 0 || formatData->precision != 0)) {
    valueStr[0] = '0';
    formatData->precision--;
    formatData->counter++;
  } else {
    int i = numberOfSymbolsInUnsigned(value) - 1;
    formatData->precision -= (i + 1);
...
int decToHexOct(char *valueStr, unsigned long long value,
  ...
  if (value == 0 &&
    ...
    // Handle special case when value is 0 and precision is specified or not
    tempStr[i++] = (base == 8) ? '0' : '0';  // '0' for octal, '0x' for hex
  } else {
  ...
  // Reverse the temporary string to get the correct representation
  i--;
  int k = 0;
  while (i >= 0) {
...
void handleSpacesPrefix(char **str, struct FormatData *formatData,
                        unsigned long long value) {
  // Проверяем, если спецификатор не 'o'
  if (formatData->specifier != 'o') {
    // Если флаг минуса установлен
    if (formatData->flags.minus != 0) {
      handleGridFlag(str, formatData, value);
    } else {
      handleGridFlagWithSpaces(str, formatData, value);
    }
  } else {
    // Если спецификатор 'o'
    if (formatData->flags.minus != 0) {
      handleOctalGridFlag(str, formatData, value);
    } else {
      handleOctalGridFlagWithSpaces(str, formatData, value);
    }
  }
}
...
void handleOctalGridFlag(char **str, struct FormatData *formatData,
                         unsigned long long value) {
  if (formatData->flags.grid == 1 && formatData->precision < 0 && value != 0) {
    formatData->width--;
    formatData->counter++;
    s21_strcat(*str, "0");
  }
